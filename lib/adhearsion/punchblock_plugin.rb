module Adhearsion
  class PunchblockPlugin < Plugin
    ...
    config :punchblock do
      ...
      connection_timeout  60               , :transform => Proc.new {|v| validate_number(v)}, :desc => "The amount of time to wait for a connection"
      reconnect_attempts  1.0/0.0          , :transform => Proc.new {|v| validate_number(v)}, :desc => "The number of times to (re)attempt connection to the server"
      reconnect_timer     5                , :transform => Proc.new {|v| validate_number(v)}, :desc => "Delay between connection attempts"
    end
