require 'spec_helper'
require 'adhearsion/menu_state_machine/calculated_match'

module CalculatedMatchCollectionTestHelper
  def mock_with_potential_matches(potential_matches)
    Adhearsion::CalculatedMatch.new :potential_matches => potential_matches
  end

  def mock_with_exact_matches(exact_matches)
    Adhearsion::CalculatedMatch.new :exact_matches => exact_matches
  end
...
describe 'CalculatedMatch' do
  it "should make accessible the context name" do
    Adhearsion::CalculatedMatch.new(:match_payload => :foobar).match_payload.should be :foobar
  end
  it "should make accessible the original pattern" do
    Adhearsion::CalculatedMatch.new(:pattern => :something).pattern.should be :something
  end
  it "should make accessible the matched query" do
    Adhearsion::CalculatedMatch.new(:query => 123).query.should be 123
  end
  it '#type_of_match should return :exact, :potential, or nil' do
    Adhearsion::CalculatedMatch.new(:potential_matches => [1]).type_of_match.should be :potential
    Adhearsion::CalculatedMatch.new(:exact_matches => [3,3]).type_of_match.should be :exact
    Adhearsion::CalculatedMatch.new(:exact_matches => [8,3], :potential_matches => [0,9]).type_of_match.should be :exact
    Adhearsion::CalculatedMatch.new.type_of_match.should be nil
  end
  it '#exact_match? should return true if the match was exact' do
    Adhearsion::CalculatedMatch.new(:exact_matches => [0,3,5]).exact_match?.should be true
  end

  it '#potential_match? should return true if the match was exact' do
    Adhearsion::CalculatedMatch.new(:potential_matches => [88,99,77]).potential_match?.should be true
  end

  it '#exact_matches should return an array of exact matches' do
    Adhearsion::CalculatedMatch.new(:exact_matches => [0,3,5]).exact_matches.should == [0,3,5]
  end

  it '#potential_matches should return an array of potential matches' do
    Adhearsion::CalculatedMatch.new(:potential_matches => [88,99,77]).potential_matches.should == [88,99,77]
  end

  it '::failed_match! should return a match that *really* failed' do
    failure = Adhearsion::CalculatedMatch.failed_match! 10..20, 30, :match_payload_does_not_matter
...
describe 'CalculatedMatchCollection' do
  ...
  before(:each) do
    @collection = Adhearsion::CalculatedMatchCollection.new
  end
