...
module StandardDispatcherBehavior
  def xtest_standard_dispatcher_behavior
    ...
    returned_event_command = Adhearsion::DSL::Dialplan::EventCommand.new "ilikesexypants"
    linking_event_command = Adhearsion::DSL::Dialplan::EventCommand.new "ihavesexypants" do
...
class NilDispatcher < Adhearsion::DSL::Dialplan::CommandDispatcher
  def dispatch!(string)
    # Would send it off to the PBX here (or prepend something to the String)
  end
end

class EvalDispatcher < Adhearsion::DSL::Dialplan::CommandDispatcher
  def dispatch!(event)
    eval event.app.to_s
  end
end
