...
describe "RouteRule assembling with one provider definition" do
  ...
  before :each do
    @provider = Adhearsion::DSL::DialingDSL::ProviderDefinition.new(:icanhascheezburger?)
  end
  ...
  before(:each) do
    1.upto 3 do |i|
      instance_variable_set "@prov_#{i}",
        Adhearsion::DSL::DialingDSL::ProviderDefinition.new("prov_#{i}")
...
describe "The DialingDSL class internals" do
  ...
  it "should allow Asterisk-like patterns" do
    Adhearsion::DSL::DialingDSL.class_eval do
      _('23XX')
  ...
  it "should define all of the VoIP constants" do
    lambda do
      Adhearsion::Constants.constants.each do |constant|
        eval "Adhearsion::DSL::DialingDSL::#{constant}"
      end
...
describe "The provider DSL's 'provider' macro" do
  ...
  it "should make the providers available in the order they were received" do
    provider_holder = Class.new Adhearsion::DSL::DialingDSL
  ...
  it "should define a singleton method for the provider name given" do
    dsl_class = Class.new(Adhearsion::DSL::DialingDSL)
  ...
  it "should not define a class-wide method for the provider name given" do
    ...
    Adhearsion::DSL::DialingDSL.should_not respond_to(:icanhascheezburger?)
  end
  it "should yield a ProviderDefinition object" do
    possible_provider_definition = nil
    Class.new(Adhearsion::DSL::DialingDSL).class_eval do
      provider(:blah) { |pd| possible_provider_definition = pd }
    end
    possible_provider_definition.should be_a_kind_of(Adhearsion::DSL::DialingDSL::ProviderDefinition)
  end
...
describe "The provider DSL's monkey patches to Regexp" do
  it "should return a RouteRule when using Regexp#| with another Regexp" do
    (// | //).should be_a_kind_of(Adhearsion::DSL::DialingDSL::RouteRule)
  end
  ...
  it "should return a RouteRule when Regexp#| is given a RouteRule" do
    route_rule = Adhearsion::DSL::DialingDSL::RouteRule.new :patterns => //
    (// | route_rule).should be_a_kind_of(Adhearsion::DSL::DialingDSL::RouteRule)
  end
  it "should return a RouteRule when Regexp#>> is given a ProviderDefinition" do
    (/1/ >> Adhearsion::DSL::DialingDSL::ProviderDefinition.new(:foo)).
      should be_a_kind_of(Adhearsion::DSL::DialingDSL::RouteRule)
  end
  it "should return a RouteRule when Regexp#>>() receives a ProviderDefinition" do
    definition = Adhearsion::DSL::DialingDSL::ProviderDefinition.new(:london)
    (/foo/ >> definition).should be_a_kind_of(Adhearsion::DSL::DialingDSL::RouteRule)
  end
...
describe "A RouteRule" do
  it "should expose chained ProviderDefinitions properly" do
    one, two, three = definitions = %w"foo bar qaz".map do |name|
      Adhearsion::DSL::DialingDSL::ProviderDefinition.new name
    end
    route = /^911$/ >> one >> two >> three
    route.should be_a_kind_of(Adhearsion::DSL::DialingDSL::RouteRule)
    route.providers.should == definitions
  end
  it "should define attr_readers for providers and patterns" do
    route = Adhearsion::DSL::DialingDSL::RouteRule.new
...
describe "The provider order modifiers" do
   it "should support randomizing trunks", :ignore => true do
    dsl = Class.new Adhearsion::DSL::DialingDSL
    dsl.class_eval do
      provider(:l33thost) {}
      provider(:h4xxhost) {}
    ...
    dsl.routes.first.provider_definitions.first.should be_a_kind_of(
      Adhearsion::DSL::DialingDSL::RandomRouteRule
    )
  end
end
...
describe "A ProviderDefinition" do
  it "should store any arbitrary property" do
    definition = Adhearsion::DSL::DialingDSL::ProviderDefinition.new :foo
    definition.qaz = :something
    definition.qaz.should == :something
  end
  it "should define a >>() method" do
    Adhearsion::DSL::DialingDSL::ProviderDefinition.new(:arghh).should respond_to(:>>)
  end

  it "should format numbers properly" do
    definition = Adhearsion::DSL::DialingDSL::ProviderDefinition.new(:arghh)
...
# @example = Class.new Adhearsion::DSL::DialingDSL
