...
describe "A EventCommand" do
  ...
  it "should allow a block that's set to response_block()" do
    block = lambda {}
    cmd = Adhearsion::DSL::Dialplan::EventCommand.new "foo", &block
    cmd.response_block.should be(block)
  end

  it "should return specified return type with returns()" do
    return_type = Numeric
    cmd = Adhearsion::DSL::Dialplan::EventCommand.new "foo", :returns => return_type
...
describe "The abstract CommandDispatcher" do
  ...
  it "should make an attribute reader for context()" do
    context = "foo"
    d = Adhearsion::DSL::Dialplan::CommandDispatcher.new String, context
    d.context.should be(context)
  end

  it "should instantiate a new instance of the specified factory" do
    d = Adhearsion::DSL::Dialplan::CommandDispatcher.new Hash
  ...
  it "should pass the context to the factory when instantiating it" do
    ...
    d = Adhearsion::DSL::Dialplan::CommandDispatcher.new klass, context
  end

  it "should not allow calling dispatch!() directly" do
    dispatcher = Adhearsion::DSL::Dialplan::CommandDispatcher.new MyFactory
    lambda do
  ...
  it "should pass a method and any args onto its CommandFactory" do
    ...
    bogus_command = Adhearsion::DSL::Dialplan::EventCommand.new "123"
  ...
  it "should continue executing response_blocks until nil is returned" do
    ...
    response = Adhearsion::DSL::Dialplan::EventCommand.new "1" do |response|
      if response > target_executions
        nil
      else
        actual_executions += 1
        Adhearsion::DSL::Dialplan::EventCommand.new(response + 1)
      end
    end
