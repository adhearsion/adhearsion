    ...
    module MenuDSL
      ...
      describe CalculatedMatch do
        ...
        it "should make accessible the original pattern" do
          calculated_match.new(:pattern => :something).pattern.should be :something
        end

        it "should make accessible the matched query" do
          calculated_match.new(:query => 123).query.should be 123
        end

        it "#type_of_match should return :exact, :potential, or nil" do
          calculated_match.new(:potential_matches => [1]).type_of_match.should be :potential
          calculated_match.new(:exact_matches => [3,3]).type_of_match.should be :exact
          calculated_match.new(:exact_matches => [8,3], :potential_matches => [0,9]).type_of_match.should be :exact
        end

        it "#exact_match? should return true if the match was exact" do
          calculated_match.new(:exact_matches => [0,3,5]).exact_match?.should be true
        end

        it "#potential_match? should return true if the match was exact" do
          calculated_match.new(:potential_matches => [88,99,77]).potential_match?.should be true
        end

        it "#exact_matches should return an array of exact matches" do
          calculated_match.new(:exact_matches => [0,3,5]).exact_matches.should == [0,3,5]
        end

        it "#potential_matches should return an array of potential matches" do
          calculated_match.new(:potential_matches => [88,99,77]).potential_matches.should == [88,99,77]
        end

        it "::failed_match! should return a match that *really* failed" do
          failure = calculated_match.failed_match! 10..20, 30, :match_payload_does_not_matter
